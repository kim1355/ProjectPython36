...
if __name__ == '__main__':
    ...
    title = bf2.div.div.h1
    pagenum = bf2.find_all(class_='size')
    pagenum = BeautifulSoup(str(pagenum), 'lxml').span
    pagepattern = re.compile('页数：(\d+)页')
    num = int(pagepattern.findall(str(pagenum))[0])
    print('文章标题：%s' % str(title))
    print('文章页数：%d' % num)
